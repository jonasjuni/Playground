...
//:
let mappedNumbersLong = numbers.map({(number: Int) -> Int in  return 3 * number})
let mappedNumbers = numbers.map({ number in 3 * number })
let mappedNumbersShort = numbers.map { $0 * 3 }

print(mappedNumbersShort)
...
let test = numbers.sorted(by:{a, b in a > b });
print(test)

func makeAdder(num: Int) -> (Int)->Int {
    func addBy(value: Int) -> Int{
        return value + num
    }
    return addBy
}

let addBy5 = makeAdder(num: 5)
print(addBy5(10))

let anonyousFunc = {(msg: String) in "!!! \(msg.uppercased()) !!!"}

print(anonyousFunc("oi tudo bem"))
